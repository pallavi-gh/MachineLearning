...
for j=[1:100,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000,1050,1100,1150,1200,1250,1300,1350,1400,1450,1500,1550,1600]
    for i=1:1688
        OrigImg = imgs(:,i);                     % Grab image 20
        Projection = Vecs(:,1:j)'*(OrigImg - Psi);    % Project onto ev's 1-10
        Reconstruction = Vecs(:,1:j)*Projection+Psi;  % Reconstruct from projection
        ...
        newsegmentedimages{i}= Reconstruction;
    end
    disp('newsegimg');size(newsegmentedimages)
    newimgdata=[];
    for k = 1:length(newsegmentedimages)
        newimgdata = [newimgdata ; im2Array(newsegmentedimages{1,k},1)];
    end 
    disp('newimgdata');size(newimgdata)   
    % Classification of new segmented images
    [partone parttwo] = breakArrayNFold(size(newimgdata,1),5,1);
    newtraindata = newimgdata(partone,:);
    trainlabels = segmentedimageslabel(partone,:);
    disp('newtraindata'); 
    size(newtraindata)
    newtestdata = newimgdata(parttwo,:);
    disp('newtestdata');size(newtestdata)
    testlabels = segmentedimageslabel(parttwo,:);
    svmModel = trainSVM(newtraindata,trainlabels,2);
    newpredictedLabels = classifySVM(svmModel,newtestdata);
    pca_accuracy(j,:)= sum(double(newpredictedLabels==testlabels'))/length(testlabels)
    %cv_accuracy{j}=crossValidation( newtraindata, trainlabels, @classifySVM, @trainSVM, 10)
end
%pca_accuracy_vector=cell2mat(pca_acuracy);
%cv_accuracy_vector=cell2mat(cv_accuracy);

ind=find(pca_accuracy);
figure(4)
plot([1:100,200,250,300,350,400,450,500,550,600,650,700,750,800,850,900,950,1000,1050,1100,1150,1200,1250,1300,1350,1400,1450,1500,1550,1600],pca_accuracy(ind),'x-');
%figure(5)
%plot(cv_accuracy_vector,Vals);
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~PCA ends~~~~~~~~~~~~~~~~~~~~~
